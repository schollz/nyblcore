	...
	router.POST("/", func(c *gin.Context) {
		...
		for _, file := range files {
			...
			if strings.Contains(filename, ".wav") || strings.Contains(filename, ".flac") || strings.Contains(filename, ".aif") || strings.Contains(filename, ".ogg") {
				...
				cmdString := []string{"sox", ftemp.Name(), "-r", "4200", "-c", "1", "-b", "8", ftemp2.Name(), "norm", "lowpass", "2100", "trim", "0", secondsString, "dither"}
				fmt.Println(cmdString)
				if len(files) > 1 && crossfade > 0 {
					cmdString = []string{"sox", ftemp.Name(), "-r", "4200", "-c", "1", "-b", "8", ftemp2.Name(), "norm", "lowpass", "2100", "trim", "0", secondsString, "dither", "fade", "h", crossfadeTimeString, "-0", crossfadeTimeString}
				}
